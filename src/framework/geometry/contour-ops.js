import { reverseContour, closeContourGap } from "./profile.js";
...
import { liftProfile, contourIsCCW, contourCorners, cubicAt, cubicDeriv } from "./contour-corners.js";

// Re-exported so existing importers (test/contour-ops-lift.test.js and others) keep
// working unchanged — the definition lives in profile.js (the contour-IR home) so
// paper-bridge.js can use it without importing this module (which already imports
// paper-bridge.js; importing back would cycle).
export { reverseContour };

// The paper-free corner/tangent/lift math now lives in contour-corners.js — so the oracle's
// shape-probe.js can use profileCorners()/cubicAt() without pulling in paper-core through
// this module's own paper-bridge.js import below — re-exported here so every existing
// importer of these names from contour-ops.js keeps working unchanged.
export { liftProfile, contourIsCCW, SMOOTH_JOINT_DEG, segTangent, jointTangents, profileCorners, cubicAt } from "./contour-corners.js";
