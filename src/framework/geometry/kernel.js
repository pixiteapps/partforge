 ...
 * @property {(o:{profile:number[][]|{start:number[],segments:object[]},path:number[][],closed?:boolean,cornerRadius?:number,ruled?:boolean,smooth?:boolean}) => Solid} sweep   sweep a 2-D profile along a 3-D polyline; legacy (profile,path,opts) accepted for now (see file header)
 * @property {(o:{profile:number[][]|{start:number[],segments:object[]}|Shape2D,degrees?:number}) => Solid} revolve   revolve a lathe profile [[r,z],…] around Z; legacy (points,opts) accepted for now (see file header)
